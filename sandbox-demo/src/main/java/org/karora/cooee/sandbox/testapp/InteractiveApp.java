...
public class InteractiveApp extends ApplicationInstance {
     ...
     * whether the <code>org.karora.cooee.demoserver</code> system property is
     * set.
     */
    ...
    static {
        ...
        try {
            if ("true".equals(System.getProperties().getProperty("org.karora.cooee.demoserver"))) {
                liveDemoServer = true;
            } else {
    ...
    public void toggleConsoleVisible() {
        ...
        // Lazy instantiate the console
        if (console == null)
            console = new ConsoleWindowPane();

        // Visibility logic
        boolean visible = console.getParent() != null;
        
        if(visible) 
            getDefaultWindow().getContent().remove(console);
        else
    ...
    public Window init() {
        ...
        mainWindow.setTitle("Karora Cooee Sandbox Demo Application");
        mainWindow.setContent(new WelcomePane());

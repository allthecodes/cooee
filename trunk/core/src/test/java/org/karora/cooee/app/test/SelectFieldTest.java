...
/**
 * Unit tests for the <code>org.karora.cooee.app.SelectField</code> component.
 */
public class SelectFieldTest extends TestCase {
    ...
    public void testProperties() {
        SelectField selectField = new SelectField();
        selectField.setActionCommand("action!");
        selectField.setBorder(Constants.BORDER_THICK_ORANGE);
        selectField.setHeight(Constants.EXTENT_30_PX);
        selectField.setInsets(Constants.INSETS_1234);
        selectField.setRolloverEnabled(true);
        selectField.setRolloverBackground(Color.GREEN);
        selectField.setRolloverFont(Constants.MONOSPACE_12);
        selectField.setRolloverForeground(Color.YELLOW);
        selectField.setWidth(Constants.EXTENT_100_PX);
        assertEquals("action!", selectField.getActionCommand());
        assertEquals(Constants.BORDER_THICK_ORANGE, selectField.getBorder());
        assertEquals(Color.GREEN, selectField.getRolloverBackground());
        assertEquals(true, selectField.isRolloverEnabled());
        assertEquals(Constants.MONOSPACE_12, selectField.getRolloverFont());
        assertEquals(Color.YELLOW, selectField.getRolloverForeground());
        assertEquals(Constants.EXTENT_30_PX, selectField.getHeight());
        assertEquals(Constants.INSETS_1234, selectField.getInsets());
        assertEquals(Constants.EXTENT_100_PX, selectField.getWidth());
    }
}

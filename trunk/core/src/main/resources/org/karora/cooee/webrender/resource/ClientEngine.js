...
EchoClientAnalyzer.analyze = function() {
    ...
    // Capture Browser Display Information
    if( typeof( window.innerWidth ) == 'number' ) {
        //Non-IE (Gecko Browsers...)
    ...
    EchoClientAnalyzer.setIntegerProperty(messagePartElement, "windowWidth", browserWidth);
    EchoClientAnalyzer.setIntegerProperty(messagePartElement, "windowHeight", browserHeight);
    EchoClientAnalyzer.setIntegerProperty(messagePartElement, "utcOffset", 0 - parseInt((new Date()).getTimezoneOffset()));
    
    // Safari Ampersand fix
 ...
 * does not exist.
 *
 * Loading Status can be provided by the Server if the "customLoadStatus" attribute is true
 *
 */
EchoClientEngine.renderLoadStatus = function() {
    ...
    else if (EchoClientEngine.renderCustomLoadStatus) {
    	// call the custom renderer if any
    	EchoClientEngine.renderCustomLoadStatus(loadStatusDivElement);
    }
    else if (loadStatusDivElement.getAttribute("customLoadStatus"))
    {
        return; // Do nothing - HTML should already be inside the div and on the screen
    }
    else
    {
        var text = "";
...
EchoClientMessage.reset = function() {
    ...
    EchoClientMessage.messageDocument 
            = EchoDomUtil.createDocument("http://www.karora.org/products/echo2/climsg", "client-message");
};
 ...
 *        elements will be fixed recursively)
 */
EchoDomUtil.fixSafariEscaping = function(node) {
 ...
 * the out going XML.
 */
EchoDomUtil.escapeText = function(text)
{

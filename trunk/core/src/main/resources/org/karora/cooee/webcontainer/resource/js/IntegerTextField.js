/*
 *  IntegerTextField is actually a proxy object for an HTML 'input'.  This object
...
IntegerTextField = function(elementId) {
...
IntegerTextField.prototype.init = function() {
    ...
    //fixed enabled state of IntegerTextField
    ...
    this.regexp = new RegExp("^[-]?[0-9]{0,10}$");
    ...
    // Event handlers: 'ActiveTextField.processXXX' refers to a static JS function
    
    EchoEventProcessor.addHandler(input, "blur",  "ActiveTextField.processBlur");
    EchoEventProcessor.addHandler(input, "focus", "ActiveTextField.processFocus");
    EchoEventProcessor.addHandler(input, "keyup", "ActiveTextField.processKeyUp");
 	EchoEventProcessor.addHandler(input, "change","ActiveTextField.processChange");
   
    //EchoDomUtil.addEventListener(input, "keypress", ActiveTextField.processKeyPress, false);
...
IntegerTextField.prototype.dispose = function() {
    ...
    //EchoDomUtil.removeEventListener(input, "keypress", ActiveTextField.processKeyPress, false);    
...
IntegerTextField.prototype.getElement = function() {
    return document.getElementById(this.elementId);
};

IntegerTextField.prototype.getInput = function() {
...
IntegerTextField.prototype.doAction = function() {
...
IntegerTextField.prototype.doValidationTransition = function() {
...
IntegerTextField.prototype.processBlur = function(echoEvent) {
...
IntegerTextField.prototype.processFocus = function(echoEvent) {
...
IntegerTextField.prototype.processKeyPress = function(e) {
// 
 ...
 *        <code>EchoEventProcessor</code>
 */
IntegerTextField.prototype.processKeyUp = function(echoEvent) {
...
IntegerTextField.prototype.processChange = function(echoEvent) {
...
IntegerTextField.prototype.validateText = function () {
...
IntegerTextField.prototype.validateRange = function () {
...
IntegerTextField.prototype.updateClientMessage = function() {
    var input = this.getInput();
...
IntegerTextField.MessageProcessor = function() { };
...
IntegerTextField.MessageProcessor.process = function(messagePartElement) {
    ...
    for (var i = 0; i<messagePartElement.childNodes.length; ++i) {
        ...
        if (messagePartElement.childNodes[i].nodeType == 1) {  // Node.ELEMENT_NODE = 1
            ...
            case "init":
                IntegerTextField.MessageProcessor.processInit(messagePartElement.childNodes[i]);
                break;
                
            case "dispose":
                ActiveTextField.MessageProcessor.processDispose(messagePartElement.childNodes[i]);
                break;
                
            case "set-text":
                ActiveTextField.MessageProcessor.processSetText(messagePartElement.childNodes[i]);
                break;
                
            case "set-foreground":
                ActiveTextField.MessageProcessor.processSetForeground(messagePartElement.childNodes[i]);
                break;
                
            case "set-background":
                ActiveTextField.MessageProcessor.processSetBackground(messagePartElement.childNodes[i]);
                break;
                
            case "set-enabled":
                ActiveTextField.MessageProcessor.processSetEnabled(messagePartElement.childNodes[i]);
                break;
...
IntegerTextField.MessageProcessor.processInit = function(initMessageElement) {

    for (var item = initMessageElement.firstChild; item; item = item.nextSibling) {
    
        var elementId = item.getAttribute("eid");        
        var textComponent = new IntegerTextField(elementId);
        ...
        // IntegerTextField specific values
        textComponent.minimumValue = item.getAttribute("minimum-value") ? item.getAttribute("minimum-value") : -2147483648;
        textComponent.maximumValue = item.getAttribute("maximum-value") ? item.getAttribute("maximum-value") :  2147483647;

...
implements Serializable {
    ...
    public static final String ID_STRING = "Karora Cooee v1.1";
    ...
    public static final String THREAD_CHANGED_PROPERTY = "threads";
     ...
     * no longer relevant. A THREAD_CHANGED_PROPERTY property change is generated 
     * on the affected Application instance.
     * <p>
    ...
    {        
    	...
    	{
    		Object previousValue = activeInstance.get(); 
    		// Activates then notifies
    		activeInstance.set(applicationInstance);
    		applicationInstance.firePropertyChange(THREAD_CHANGED_PROPERTY, previousValue, applicationInstance);
    	}
    	else
    	{
    		// Notifies existing active instance that it is de-activating 
    		ApplicationInstance deactivatingInstance = (ApplicationInstance)activeInstance.get();
    		activeInstance.set(applicationInstance);
    		if ( deactivatingInstance != null )
    			deactivatingInstance.firePropertyChange(THREAD_CHANGED_PROPERTY, deactivatingInstance, applicationInstance);
    	}
    }
    ...
    public Style getStyle(Class componentClass, String styleName) {
        if (getStyleSheet() == null) {
            ...
            return getStyleSheet().getStyle(componentClass, styleName);
        }
    }
    
    /**
     * Retrieves the <code>StyleSheet</code> for the <code>ApplicationInstance</code>
     * so that a client can append/change/remove styles.
     * <p>
     * Note that <code>ServerUpdateManager.processFullRefresh()</code> needs to be
     * called to refresh the application state after a <code>StyleSheet</code>
     * has been changed so that any changes to the <code>StyleSheet</code> are processed.
     * Because of this, appending to the style sheet should be
     * done sparingly, given that doing so requires the entire
     ...
     * <p> The <code>ServerUpdateManager</code> can be retrieved via the
     * <code>UpdateManager</code>.
     * @return the appropriate application-wide stylesheet, or null
     *         if none exists
     */
     ...
     * 
     * This method has been updated to address COO-64, where two
     * modal windows created in the same server transaction cause
     * the last created window to be in focus, but the first to have
     * modal control
     * 
     * @param component the <code>Component</code>

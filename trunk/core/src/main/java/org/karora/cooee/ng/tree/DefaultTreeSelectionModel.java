...
public class DefaultTreeSelectionModel implements TreeSelectionModel, Serializable {
	...
	/**
	  * Sets the selection to the paths in paths.  Even if it does not 
	  * represent a change the TreeSelectionListeners are notified.
	  * Potentially paths will be held by the reciever, in other words don't change
	...
	public void setSelectionPaths(TreePath[] pPaths) {
		...
		if ((newCount + oldCount) != 0) {
			...
			resetRowSelection();
			/* COO-140: Always notify of the change. */
			notifyPathChange(cPaths, beginLeadPath);
		}
	}
	/**

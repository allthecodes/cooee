...
import org.karora.cooee.app.layout.TabPaneLayoutData;
...
import org.karora.cooee.webcontainer.syncpeer.TabPanePeer;
...
public class TabPaneTest extends AbstractTest {
    ...
    /**
     * keep track of whether a property changed listener is added
     */
    boolean pclIsAdded = false;
    ...
    public TabPaneTest() {
        ...
        addExtentPropertyTests(TestControlPane.CATEGORY_PROPERTIES, TabPane.PROPERTY_TAB_HEIGHT, 1);
        addExtentPropertyTests(TestControlPane.CATEGORY_PROPERTIES, TabPane.PROPERTY_TAB_WIDTH, 1);
        ...
        final PropertyChangeListener pcl = new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent pce) {
				if (pce.getPropertyName() == TabPane.ACTIVE_TAB_INDEX_CHANGED_PROPERTY) {
					InteractiveApp.getApp().consoleWrite("Tab #" + tabPane.getActiveTabIndex() + " is selected");
				}
			}
        	
        };
        
        testControlsPane.addButton("Property Changed Listeners", "Active Tab Index Changed = off", new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	if (pclIsAdded) {
            		tabPane.removePropertyChangeListener(pcl);
            		pclIsAdded = false;
            	}
            }
        });
        
        testControlsPane.addButton("Property Changed Listeners", "Active Tab Index Changed = on (check Console)", new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	if (!pclIsAdded) {
            		tabPane.addPropertyChangeListener(pcl);
            		pclIsAdded = true;
            	}
            }
        });

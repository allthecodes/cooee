...
ExtrasDropDownMenu.prototype.renderDropDownAdd = function() {
    ...
    if (this.height) {
        ...
        dropDownDivElement.style.height = "100%";
    ...
    relativeContainerDivElement.style.width = "100%";
    
    var expandElement = document.createElement("span");
    expandElement.id = this.elementId + "_expand";
    expandElement.style.position = "absolute";
    expandElement.style.top = "0px";
    expandElement.style.right = "0px";
    //Dropdown will fill the whole area if we don't have a selection item.
    //Will be resized when an item is selected.
    expandElement.style.width = ((this.selection && this.selectedItem) || this.staticSelection) ? "20px" : "100%";
    ...
    if(this.selection || this.staticSelection) {
        //Only add a content area if we can do selections.
        ...
        //Hide the content if there is no current selection item
        //Will be resized when an item is selected.
        if(this.selectedItem) {
            contentDivElement.style.marginRight = "20px";
        } else {
            contentDivElement.style.width = "0px";
        }
        ...
        EchoEventProcessor.addHandler(contentDivElement, "click", "ExtrasDropDownMenu.processDropDownContentClick");
    }
    
    EchoEventProcessor.addHandler(expandElement, "click", "ExtrasDropDownMenu.processDropDownClick");
    //EchoEventProcessor.addHandler(dropDownDivElement, "click", "ExtrasDropDownMenu.processDropDownClick");
    EchoEventProcessor.addHandler(dropDownDivElement, "mouseover", "ExtrasDropDownMenu.processDropDownMouseOver");
    EchoEventProcessor.addHandler(dropDownDivElement, "mouseout", "ExtrasDropDownMenu.processDropDownMouseOut");
    if (EchoClientProperties.get("browserInternetExplorer")) {
...
ExtrasDropDownMenu.prototype.renderDropDownDispose = function() {
    ...
    var contentDivElement = document.getElementById(menuId + "_content");
    var expandDivElement = document.getElementById(menuId + "_expand");

    if(expandDivElement) {
        EchoEventProcessor.removeHandler(expandDivElement, "click");
    }
    if(contentDivElement) {
        EchoEventProcessor.removeHandler(contentDivElement, "click");
    }
...
ExtrasDropDownMenu.processDropDownMouseOver = function(echoEvent) {
    ExtrasDropDownMenu.processDropDownMouseMove(echoEvent, true);
};
    
ExtrasDropDownMenu.processDropDownMouseOut = function(echoEvent) {
    ExtrasDropDownMenu.processDropDownMouseMove(echoEvent, false);
};

ExtrasDropDownMenu.processDropDownMouseMove = function(echoEvent, state) {
    var menuId = EchoDomUtil.getComponentId(echoEvent.registeredTarget.id);
    if(menuId != null)
    {
        //Get the main component and either the content or the dropdown.
	    var menu = ExtrasMenu.getComponent(menuId);
    	if(menu) {
        	menu.setDropDownHighlight(document.getElementById(menuId), state);
        }
    }
};

ExtrasDropDownMenu.prototype.setDropDownHighlight = function(menuItem, state) {
    if(menuItem) {
        if(state) {
            //Mouse over the element so used the selected style
            if (this.selectionBackgroundImage != null) {
                EchoCssUtil.applyStyle(menuItem, this.selectionBackgroundImage);
            }
            menuItem.style.backgroundColor = this.selectionBackground;
            menuItem.style.color = this.selectionForeground;
        } else {
            //Mouse left element so used the default style
            if (this.selectionBackgroundImage != null) {
                EchoCssUtil.applyStyle(menuItem, this.backgroundImage);
            }
            menuItem.style.backgroundColor = this.background;
            menuItem.style.color = this.foreground;
        }
    }
};

ExtrasDropDownMenu.processDropDownContentClick = function(echoEvent) {
    ...
    if (menu == null || !menu.enabled || !EchoClientEngine.verifyInput(menuId, false)) {
        return;
    }

    if (menu.selectedItem) {
        menu.notifyServer(menu.selectedItem);
    }
};
...
ExtrasDropDownMenu.prototype.setSelection = function(menuModel) {
    ...
    if(!this.selectedItem) {
        //Changing from a menu with just a dropdown to a 
        //menu with a content and dropdown so split the element.
        contentDivElement.style.marginRight = "20px";

        var dropdownDivElement = document.getElementById(this.elementId + "_expand");
        if(dropdownDivElement) {
            dropdownDivElement.style.width = "20px";
        }
    }
...
ExtrasDropDownMenu.MessageProcessor.processInit = function(initMessageElement) {
    ...
    menu.selection = initMessageElement.getAttribute("selection") == "true";
    menu.staticSelection = initMessageElement.getAttribute("staticSelection") == "true";

...
import org.karora.cooee.app.event.TableModelEvent;
import org.karora.cooee.app.event.TableModelListener;
import org.karora.cooee.app.list.DefaultListSelectionModel;
...
import org.karora.cooee.ng.tree.DefaultTreeCellRenderer;
import org.karora.cooee.ng.tree.DefaultTreeSelectionModel;
import org.karora.cooee.ng.tree.TreeCellRenderer;
import org.karora.cooee.ng.tree.TreeExpansionEvent;
import org.karora.cooee.ng.tree.TreeExpansionListener;
import org.karora.cooee.ng.tree.TreeModelEvent;
import org.karora.cooee.ng.tree.TreeModelListener;
import org.karora.cooee.ng.tree.TreePath;
import org.karora.cooee.ng.tree.TreeSelectionEvent;
import org.karora.cooee.ng.tree.TreeSelectionListener;
import org.karora.cooee.ng.tree.TreeSelectionModel;
import org.karora.cooee.ng.treetable.AbstractTreeTableModel;
import org.karora.cooee.ng.treetable.DefaultTreeTableModel;
import org.karora.cooee.ng.treetable.TreeTableModel;

/**
 * <p>
 * <code>TreeTable</code> is a specialized
 * {@link org.karora.cooee.app.Table table} consisting of a single column in which
 ...
 * {@link org.karora.cooee.ng.treetable.TreeTableModel}. It extends the
 * {@link org.karora.cooee.ng.tree.TreeModel}interface to allow access to cell data by
 ...
 * adapter class to adapt it to a {@link org.karora.cooee.app.table.TableModel}.
...
public class TreeTable extends TableEx {
	 ...
	 */
	class ListToTreeSelectionModelWrapper extends DefaultTreeSelectionModel implements TreeSelectionListener {
		...
		public void valueChanged(TreeSelectionEvent e) {
			if (!updatingListSelectionModel) {
				updatingListSelectionModel = true;
				try {
					//Clean up our current selections
					tableSelectionModel.clearSelection();
					
					//Find all the tree items currently selected
					TreePath[] paths = getSelectionPaths();
					
					if(paths != null && paths.length > 0) {
						//Convert the paths to row numbers
						int rows[] = getTree().getRowsForPaths(paths);
					
						//Select the rows
						if(rows != null) {
							for (int i = 0; i < rows.length; i++) {
								if(rows[i] >= 0) {
									tableSelectionModel.setSelectedIndex(rows[i], true);
								}
							}
						}
					}
	...
	static class TreeTableModelAdapter extends AbstractTableModel {
		...
		/**
		 * @see org.karora.cooee.app.table.TableModel#getColumnName(int)
		 */
		 ...
		 * {@link org.karora.cooee.ng.TreeTable#setModel(TableModel)}.
		 * 
	 ...
	 * {@link org.karora.cooee.ng.treetable.DefaultTreeTableModel}.
	 */
	public TreeTable() {
    ...
    /**
     * Performs a complete refresh on the table tree - use sparingly.
     * 
     *
     */
    public void refresh()
    {
        getTreeTableModelAdapter().fireTableStructureChanged();
    }
    
    
	/**
	 * Constructs a TreeTable using the specified
	 * {@link org.karora.cooee.ng.treetable.TreeTableModel}.
	 * 
	 * @param treeModel
	 ...
	 * {@link org.karora.cooee.ng.treetable.TreeTableModel}and
	 * {@link org.karora.cooee.ng.treetable.TreeAndTableCellRenderer}. The
	 * treeAndTableCellRenderer must have been constructed using the same
	 * instance of {@link org.karora.cooee.ng.treetable.TreeTableModel}as passed to
	 * this constructor.
	 * 
	 * @param treeModel
	 *            model for the TreeTable
	 ...
	 */
	private final void init(TreeAndTableCellRenderer cellRenderer) {
		this.treeAndTableCellRenderer = cellRenderer;
		...
		this.getModel().addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				// re-select the correct selected row (in case the table structure has changed)
				ListSelectionModel w = getSelectionModel();
				w.clearSelection();
				TreePath[] paths = treeAndTableCellRenderer.getSelectionPaths();
				if(paths == null)
					return;
				
				for(int i=0; i<paths.length; ++i)
				{
	                int row = treeAndTableCellRenderer.getRowForPath(paths[i]);
	                if(row == -1 || row >= getModel().getRowCount())
	                	return;
					w.setSelectedIndex(row, true);
				}
			}
	 ...
	 *         the {@link org.karora.cooee.ng.tree.TreeNode}interface; false otherwise.
	 */
	 ...
	 * 
	 * @see org.karora.cooee.app.Table#processInput(java.lang.String,
	 *      java.lang.Object)
	 */
	public void processInput(String inputName, Object inputValue) {
	 ...
	 * {@link org.karora.cooee.ng.treetable.TreeTableModel}to make it a compatible
	 * TableModel, <b>this method should never be called directly </b>. Use
	 * {@link #setTreeTableModel(org.karora.cooee.ng.treetable.TreeTableModel) setTreeTableModel}
	 * instead.
	 * </p>
	 * 
	 * <p>
	 ...
	 * {@link org.karora.cooee.ng.treetable.TreeTableModel}. The same data model may be
	...
	/**
	 * @see org.karora.cooee.app.Table#validate()
	 */

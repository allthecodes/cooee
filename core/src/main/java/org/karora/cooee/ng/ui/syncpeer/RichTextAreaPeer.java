...
import org.karora.cooee.ng.richtext.RichTextSpellChecker;
import org.karora.cooee.ng.ui.resource.Resources;
import org.karora.cooee.ng.ui.util.CssStyleEx;
import org.karora.cooee.ng.ui.util.HtmlNodeLexer;
import org.karora.cooee.ng.ui.util.HtmlTable;
...
public class RichTextAreaPeer extends AbstractEchoPointPeer implements PropertyUpdateProcessor, ActionProcessor
{
	 ...
	 */
	public static final Service RICH_TEXT_SERVICE = JavaScriptService.forResource("EPNG.RichTextArea", "/org/karora/cooee/ng/ui/resource/js/rta.js");
	static {
		WebRenderServlet.getServiceRegistry().add(RICH_TEXT_SERVICE);
	}

	private static final ImageReference IMAGE_WIGGLY_RED_LINE = new ResourceImageReference("/org/karora/cooee/ng/resource/images/richtext/ep_rt_wiggly_redline.gif");
	...
	private String spellCheckText(final RenderContext rc, final Element directiveItem, final RichTextArea rta, String htmlText, boolean spellCheckInProgress) {
		...
		if (spellCheckInProgress) {
			...
			String newHtmlText = HtmlNodeLexer.lex(htmlText, new  HtmlNodeLexer.HtmlLexerCallBack() {
				...
				/**
				 * @see org.karora.cooee.ng.ui.util.HtmlNodeLexer.HtmlLexerCallBack#onCommentNode(java.lang.StringBuffer)
				 */
				public StringBuffer onCommentNode(StringBuffer comment) {
					return comment;
				}
				...
				/**
				 * @see org.karora.cooee.ng.ui.util.HtmlNodeLexer.HtmlLexerCallBack#onElementNode(java.lang.StringBuffer)
				 */
				public StringBuffer onElementNode(StringBuffer element) {
					return element;
				}
				/**
				 * @see org.karora.cooee.ng.ui.util.HtmlNodeLexer.HtmlLexerCallBack#onTextNode(java.lang.StringBuffer)
				 */
				public StringBuffer onTextNode(StringBuffer textNode) {
	...
	public void renderHtml(RenderingContext rc, Node parent, Component component) {
		...
		if (paragraphStyles != null || fontNames != null || fontSizes != null) {
			...
			if (!rta.isShowToolbar()) td.setAttribute("style", td.getAttribute("style") + "display: none");
		}
	...
	private void renderAllButtons(RenderingContext rc, HtmlTable tableOut, RichTextArea rta, String rtaId, String rtatbCell, String userAgent) {
		...
		for (int i = 0; i < commands.length; i++) {
			...
			String display = rta.isShowToolbar() ? "" : "display: none";             
            td.setAttribute("style", rtatbCell + display);
	...
	private boolean isCommandSupported(String command, RichTextArea rta, RichTextRenderer rtaRenderer, String userAgent) {
	    // Disable command support if not showing the toolbar.
	    if (!rta.isShowToolbar()) return false;

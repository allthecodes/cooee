...
import org.karora.cooee.ng.tree.DefaultTreeModel;
import org.karora.cooee.ng.tree.DefaultTreeSelectionModel;
import org.karora.cooee.ng.tree.EmptyTreeSelectionModel;
import org.karora.cooee.ng.tree.ExpandVetoException;
import org.karora.cooee.ng.tree.RowMapper;
import org.karora.cooee.ng.tree.TreeActionEventEx;
...
import org.karora.cooee.ng.tree.TreeWillExpandListener;
 ...
 * <p>
 * 
 * @see org.karora.cooee.ng.tree.TreeModel
 * @see org.karora.cooee.ng.tree.DefaultTreeModel
 * @see org.karora.cooee.ng.tree.TreeCellRenderer
 * @see org.karora.cooee.ng.tree.DefaultTreeCellRenderer
 * @see org.karora.cooee.ng.tree.TreeNode
 * @see org.karora.cooee.ng.tree.MutableTreeNode
 * @see org.karora.cooee.ng.tree.DefaultMutableTreeNode
 */
public class Tree extends AbleComponent {
	...
	private class TreeExpansionHandler implements Serializable, TreeExpansionListener {
		...
		/**
		 * @see org.karora.cooee.ng.tree.TreeExpansionListener#treeCollapsed(TreeExpansionEvent)
		 */
		...
		/**
		 * @see org.karora.cooee.ng.tree.TreeExpansionListener#treeExpanded(TreeExpansionEvent)
		 */
	...
	protected class TreeRowMapper implements Serializable, RowMapper {
		...
		/**
		 * @see org.karora.cooee.ng.tree.RowMapper#getRowsForPaths(org.karora.cooee.ng.tree.TreePath[])
		 */
	...
	public void processInput(String name, Object value) {
		...
		if (name.startsWith(INPUT_TOGGLE)) {
			...
			if (selectionModel != null) {
				if (selectionModel.isPathSelected(path) && this.getSelectionModel().getSelectionMode() != TreeSelectionModel.SINGLE_TREE_SELECTION) {
	 ...
	 * -1.
	 */
	public TreePath getRowForPath(final int row) {
		if (row < 0) {
			return null;
		}
	...
	public int getRowForPath(final TreePath path) {
		...
		if (nodes.length == 0 || !isExpanded(path.getParentPath()))
			return -1;

		//Used to iterate over the visible tree items until a specific
		//item is located
		class TreeRowNavigationListener implements TreePathNavigationListener {
			int targetRow = -1;

			public boolean onTreePath(TreePath treePath, int pathRow) {
				//Check to see if this is the specified component
				if (path.getLastPathComponent() == treePath.getLastPathComponent()) {
					//Store the row the component was found on.
					targetRow = pathRow;
					return false;
				}
				return true;
			...
			public int getLastEncounteredRow() {
				return targetRow;
			}
		};
		// call the method with the path listener to find the last encounted row
		TreeRowNavigationListener rowNavigationListener = new TreeRowNavigationListener();
		findAllVisibleRows(rowNavigationListener);
		return rowNavigationListener.getLastEncounteredRow();
	}
	 ...
	 * @see org.karora.cooee.ng.tree.TreeIcons
	 */
	...
	/**
	 * @see org.karora.cooee.app.Component#setEnabled(boolean)
	 */
	...
	/**
	 * @see org.karora.cooee.app.Component#setVisible(boolean)
	 */

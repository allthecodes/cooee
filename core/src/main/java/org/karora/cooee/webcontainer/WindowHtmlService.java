...
implements Service {
    ...
    /**
     * Echo/Cooee Debug Mode Identifier
     * 
     * This is a Servlet Init Parameter, that must be set via your web.xml however
    ...
    public void service(Connection conn) throws IOException {
        ...
        boolean debug = !("false".equals(conn.getServlet().getInitParameter(ECHO_DEBUG_PARAMETER)));

        
        // Get the initial document
        //BaseHtmlDocument baseDoc = new BaseHtmlDocument(ROOT_ID);
        
        BaseClientDocument baseDoc = null;
        Class baseDocumentImplementation = null;
        
        if (ci.getBaseClientDocumentClass() != null)
        {
        	baseDocumentImplementation = ci.getBaseClientDocumentClass();
        }
        else
        {
        	baseDocumentImplementation = DEFAULT_BASE_DOCUMENT_IMPLEMENTATION;
        }
        
        baseDoc = instantiateBaseDocument(baseDocumentImplementation, ci, ROOT_ID);
    ...
    private BaseClientDocument instantiateBaseDocument(Class baseDocumentImplementation, ContainerInstance ci, String rootID)
    {
    	Object instance;
    	try
    	{
    		Constructor constructor = baseDocumentImplementation.getConstructor(ContainerInstance.class, String.class);
    		instance = constructor.newInstance(ci, rootID);
    		return (BaseClientDocument) instance;
    	}
    	catch (Exception e)
    	{
    		System.err.println ("[WARNING] - Unable to instantiate class: " + baseDocumentImplementation.getName() + ", Clients will not be able to access framework");

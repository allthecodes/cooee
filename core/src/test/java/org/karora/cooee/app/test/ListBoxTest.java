...
/**
 * Unit tests for the <code>org.karora.cooee.app.ListBox</code> component.
 */
public class ListBoxTest extends TestCase {
    ...
    public void testProperties() {
        ListBox listBox = new ListBox();
        listBox.setActionCommand("action!");
        listBox.setBorder(Constants.BORDER_THICK_ORANGE);
        listBox.setHeight(Constants.EXTENT_30_PX);
        listBox.setInsets(Constants.INSETS_1234);
        listBox.setRolloverEnabled(true);
        listBox.setRolloverBackground(Color.GREEN);
        listBox.setRolloverFont(Constants.MONOSPACE_12);
        listBox.setRolloverForeground(Color.YELLOW);
        listBox.setWidth(Constants.EXTENT_100_PX);
        assertEquals("action!", listBox.getActionCommand());
        assertEquals(Constants.BORDER_THICK_ORANGE, listBox.getBorder());
        assertEquals(Color.GREEN, listBox.getRolloverBackground());
        assertEquals(true, listBox.isRolloverEnabled());
        assertEquals(Constants.MONOSPACE_12, listBox.getRolloverFont());
        assertEquals(Color.YELLOW, listBox.getRolloverForeground());
        assertEquals(Constants.EXTENT_30_PX, listBox.getHeight());
        assertEquals(Constants.INSETS_1234, listBox.getInsets());
        assertEquals(Constants.EXTENT_100_PX, listBox.getWidth());
    }
}
